...
class NormalQueueRTLCtrl(Component):
    def construct(s, num_entries):
        ...
        def comb():
            ...
            if s.enq_ptr == s.last_idx:
                s.enq_ptr_inc = 0
            else:
                s.enq_ptr_inc = s.enq_ptr + 1

            # deq ptr incrementer

            if s.deq_ptr == s.last_idx:
                s.deq_ptr_inc = 0
            else:
                s.deq_ptr_inc = s.deq_ptr + 1

            # set the next ptr value

            if s.do_enq:
                s.enq_ptr_next = s.enq_ptr_inc
            else:
                s.enq_ptr_next = s.enq_ptr

            if s.do_deq:
                s.deq_ptr_next = s.deq_ptr_inc
            else:
                s.deq_ptr_next = s.deq_ptr
            ...
            s.full_next_cycle = (
                s.do_enq and not s.do_deq and (s.enq_ptr_next == s.deq_ptr)
            )
