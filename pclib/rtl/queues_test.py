...
def run_tv_test(dut, test_vectors):
    ...
    def tv_out(dut, tv):
        if tv[1] != "?":
            assert dut.enq.rdy == tv[1]
        if tv[4] != "?":
            assert dut.deq.rdy == tv[4]
        if tv[5] != "?":
            assert dut.deq.msg == tv[5]
...
class TestHarness(Component):
    def construct(
        ...
        s.sink = TestSinkRTL(
            MsgType, sink_msgs, sink_initial, sink_interval, arrival_time
        )
...
def test_normal2_simple():
    th = TestHarness(Bits16, 2, test_msgs, test_msgs, 0, 0, 0, 0, arrival_pipe)
    run_sim(th)
