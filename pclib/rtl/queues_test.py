...
class TestHarnessNormal( Component ):
...
class TestHarnessSingle( Component ):

  def construct( s, QueueType, EntryType, src_msgs, sink_msgs, src_initial, 
                 src_interval, sink_initial, sink_interval, 
                 arrival_time=None ):

    s.src  = TestSrcCL  ( src_msgs,  src_initial,  src_interval )
    s.dut  = QueueType  ( EntryType )
    s.sink = TestSinkRTL( EntryType, sink_msgs, sink_initial, sink_interval, 
                          arrival_time )
...
def test_normal2_simple():
  th = TestHarnessNormal( Bits16, 2, test_msgs, test_msgs, 0, 0, 0, 0,
                          arrival_pipe )
  run_sim( th )
