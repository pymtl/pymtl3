...
def run_test_queue(model, test_vectors):
    ...
    def tv_out(model, tv):
        if tv[1] != "?":
            assert model.enq.rdy == tv[1]
        if tv[3] != "?":
            assert model.deq.val == tv[3]
        if tv[5] != "?":
            assert model.deq.msg == tv[5]
