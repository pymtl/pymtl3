...
class TestSinkCL(Component):
    def construct(s, msgs, initial_delay=0, interval_delay=0, arrival_time=None):
        ...
        s.cycle_count = 0
        s.msgs = list(msgs)
        s.arrival_time = None if arrival_time is None else list(arrival_time)
        ...
        s.add_constraints(
            U(up_sink_count) < M(s.recv), U(up_sink_count) < M(s.recv.rdy)
        )
...
Expected cycles: {}
Received at    : {}""".format(
                    s.arrival_time[s.idx], s.cycle_count
                )
            )
        else:
            s.idx += 1
...
class TestSinkRTL(Component):
    def construct(
        s, MsgType, msgs, initial_delay=0, interval_delay=0, arrival_time=None
    ):
        ...
        s.sink = TestSinkCL(msgs, initial_delay, interval_delay, arrival_time)
    ...
    def line_trace(s):
        return "|{}|->{}".format(s.adapter.line_trace(), s.sink.line_trace())
