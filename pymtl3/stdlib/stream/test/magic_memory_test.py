...
class TestHarness( Component ):
  ...
  def construct( s, cls, nports, PortTypes, src_msgs, sink_msgs,
                 stall_prob, mem_extra_latency, src_initial,  src_interval, sink_initial, sink_interval,
                 arrival_time=None ):
    assert len(PortTypes) == nports
    s.srcs = [ SourceRTL( PortTypes[i][0], src_msgs[i], src_initial, src_interval )
                for i in range(nports) ]
    s.mem  = cls( nports, PortTypes, stall_prob, mem_extra_latency )
    s.sinks = [ SinkRTL( PortTypes[i][1], sink_msgs[i], sink_initial, sink_interval,
                            arrival_time ) for i in range(nports) ]

    # Connections
    for i in range(nports):
      s.srcs[i].send    //= s.mem.ifc[i].req
      s.mem.ifc[i].resp //= s.sinks[i].recv
...
test_case_table = mk_test_case_table([
  (                                  "msg_func          stall extra_lat src_init src_intv sink_init sink_intv"),
  ...
  [ "stream_stall0.5_extralat0",      stream_msgs,      0.5,  0,        0,       0,       0,        0         ],
  [ "stream_stall0.0_extralat4",      stream_msgs,      0.0,  4,        0,       0,       0,        0         ],
  [ "stream_stall0.5_extralat4",      stream_msgs,      0.5,  4,        0,       0,       0,        0         ],
  [ "random_stall0.5_extralat4_3x14", random_msgs,      0.5,  4,        5,       14,      7,        14        ],
])
...
def test_1port( test_params, cmdline_opts ):
  ...
  run_sim( TestHarness( MagicMemoryRTL, 1, [(req_cls, resp_cls)],
                        [ msgs[::2] ],
                        [ msgs[1::2] ],
                        test_params.stall, test_params.extra_lat,
                        test_params.src_init, test_params.src_intv,
                        test_params.sink_init, test_params.sink_intv ), cmdline_opts )
...
def test_2port( test_params, cmdline_opts ):
  ...
  run_sim( TestHarness( MagicMemoryRTL, 2, [(req_cls, resp_cls)]*2,
                        [ msgs0[::2],  msgs1[::2]  ],
                        [ msgs0[1::2], msgs1[1::2] ],
                        test_params.stall, test_params.extra_lat,
                        test_params.src_init, test_params.src_intv,
...
def test_20port( test_params, cmdline_opts ):
  ...
  run_sim( TestHarness( MagicMemoryRTL, 20, [(req_cls, resp_cls)]*20,
                        [ x[::2]  for x in msgs ],
                        [ x[1::2] for x in msgs ],
                        test_params.stall, test_params.extra_lat,
                        test_params.src_init, test_params.src_intv,
...
def test_read_write_mem( cmdline_opts ):
  ...
  th = TestHarness( MagicMemoryRTL, 2, [(req_cls, resp_cls)]*2, [msgs[::2], []], [msgs[1::2], []],
                    0, 0, 0, 0, 0, 0 )
  th.elaborate()
