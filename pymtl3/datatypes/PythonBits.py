...
class Bits:
  ...
  def __rsub__( self, other ):
    ...
    # Shouldn't be Bits
    up = _upper[ nbits ]
    return _new_valid_bits( nbits, (int(other) - self._uint) & up )
    # other = int(other)
    # if other < 0 or other > up:
    #   raise ValueError( f"Integer {hex(other)} is not a valid binop operand with Bits{nbits}!\n"
    #                     f"Suggestion: 0 <= x <= {hex(up)}" )
  ...
  def __and__( self, other ):
    nbits = self._nbits
    return _new_valid_bits(nbits, self._uint & int(other))
    # if hasattr( other, 'nbits' ) and hasattr( other, '_uint' ):
    #   if other.nbits != nbits:
  ...
  def __rfloordiv__( self, other ):
    nbits = self._nbits
    return _new_valid_bits( nbits, int(other) // self._uint )
    # other = int(other)
    # up = _upper[ nbits ]
    # if other < 0 or other > up:
    #   raise ValueError( f"Integer {hex(other)} is not a valid binop operand with Bits{nbits}!\n"
