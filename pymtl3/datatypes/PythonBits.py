...
class Bits:
    ...
    def __init__(self, nbits, v=0, trunc_int=False):
        ...
        if isinstance(v, Bits):
            if nbits != v.nbits:
                if nbits < v.nbits:
                    raise ValueError(
                        f"The Bits{v.nbits} object on RHS is too wide to be used to construct Bits{nbits}!\n"
                        f"- Suggestion: directly use trunc( value, {nbits}/Bits{nbits} )"
                    )
                else:
                    raise ValueError(
                        f"The Bits{v.nbits} object on RHS is too narrow to be used to construct Bits{nbits}!\n"
            ...
            if not trunc_int:
                ...
                if v < lo or v > up:
                    raise ValueError(
                        f"Value {hex(v)} is too wide for Bits{nbits}!\n"
    ...
    def __ilshift__(self, v):
        ...
        try:
            ...
            if v.nbits != nbits:
                if v.nbits < nbits:
                    raise ValueError(
                        f"Bitwidth of LHS must be equal to RHS during <<= non-blocking assignment, "
            ...
            if v < lo or v > up:
                raise ValueError(
                    f"RHS value {hex(v)} of <<= is too wide for LHS Bits{nbits}!\n"
                    f"(Bits{nbits} only accepts {hex(lo)} <= value <= {hex(up)})"
                )
            self._next = v & up
    ...
    def __imatmul__(self, v):
        ...
        try:
            ...
            if v.nbits != nbits:
                if v.nbits < nbits:
                    raise ValueError(
                        f"Bitwidth of LHS must be equal to RHS during @= blocking assignment, "
                        f"but here LHS Bits{nbits} > RHS Bits{v.nbits}.\n"
                        f"- Suggestion: LHS @= zext/sext(RHS, nbits/Type)"
                    )
                else:
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, slice):
            ...
            try:
                ...
                raise IndexError(
                    f"Invalid access: [{idx.start}:{idx.stop}] in a Bits{self._nbits} instance"
                )
    ...
    def __setitem__(self, idx, v):
        ...
        if isinstance(idx, slice):
            ...
            if isinstance(v, Bits):
                if v.nbits != slice_nbits:
                    if v.nbits < slice_nbits:
                        raise ValueError(
                            f"Cannot fit a Bits{v.nbits} object into a {slice_nbits}-bit slice [{start}:{stop}]\n"
                ...
                if v < lo or v > up:
                    raise ValueError(
                        f"Cannot fit {v} into a Bits{slice_nbits} slice\n"
        ...
        if isinstance(v, Bits):
            if v.nbits > 1:
                raise ValueError(
                    f"Cannot fit a Bits{v.nbits} object into the 1-bit slice"
                )
    ...
    def __add__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '+' (add) operation must have matching bitwidth, "
            ...
            if other < 0 or other > up:
                raise ValueError(
                    f"Integer {hex(other)} is not a valid binop operand with Bits{nbits}!\n"
    ...
    def __sub__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '-' (sub) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, (self._uint - other._uint) & _upper[nbits])
        except AttributeError:
    ...
    def __mul__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '*' (mul) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, (self._uint * other._uint) & _upper[nbits])
        except AttributeError:
    ...
    def __and__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '&' (and) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
    ...
    def __or__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '|' (or) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, self._uint | other._uint)
        except AttributeError:
    ...
    def __xor__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of  '^' (xor) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, self._uint ^ other._uint)
        except AttributeError:
    ...
    def __floordiv__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '//' (div) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, (self._uint // other._uint) & _upper[nbits])
        except AttributeError:
    ...
    def __mod__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '%' (mod) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, (self._uint % other._uint) & _upper[nbits])
        except AttributeError:
    ...
    def __lshift__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '<<' (lshift) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            uint = other._uint
            if uint >= nbits:
    ...
    def __rshift__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '>>' (rshift) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(nbits, self._uint >> other._uint)
        except AttributeError:
    ...
    def __eq__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '==' (eq) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
    ...
    def __ne__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '!=' (ne) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(1, self._uint != other._uint)
        except AttributeError:
    ...
    def __lt__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '<' (lt) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(1, self._uint < other._uint)
        except AttributeError:
            ...
            if other < 0 or other > _upper[self._nbits]:
                raise ValueError(
                    f"Integer {hex(other)} is not a valid binop operand with Bits{self._nbits}!\n"
    ...
    def __le__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '<=' (le) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(1, self._uint <= other._uint)
        except AttributeError:
    ...
    def __gt__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '>' (gt) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(1, self._uint > other._uint)
        except AttributeError:
    ...
    def __ge__(self, other):
        ...
        try:
            if other.nbits != nbits:
                raise ValueError(
                    f"Operands of '>=' (ge) operation must have matching bitwidth, "
                    f"but here Bits{nbits} != Bits{other.nbits}.\n"
                )
            return _new_valid_bits(1, self._uint >= other._uint)
        except AttributeError:
    ...
    def __repr__(self):
        return "Bits{}(0x{})".format(
            self._nbits,
            "{:x}".format(int(self._uint)).zfill(((self._nbits - 1) // 4) + 1),
        )
