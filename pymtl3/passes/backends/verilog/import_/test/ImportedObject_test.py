...
from ... import VerilatorImportConfigs, VerilatorImportPass
...
def local_do_test( _m ):
  ...
  _m.config_verilog_import.fill_missing( _m )
  ...
  class VReg( Component ):
    def construct( s ):
      ...
      s.config_verilog_import = VerilatorImportConfigs(
          vl_src = get_dir(__file__)+'VReg.v',
          port_map = {
            "clk" : "clk",
            "reset" : "reset",
            "in_" : "d",
...
def test_vl_uninit( do_test ):
  ...
  class VUninit( Component ):
    def construct( s ):
      ...
      s.out = OutPort( Bits32 )
      s.config_verilog_import = VerilatorImportConfigs(
          vl_src = get_dir(__file__)+'VUninit.v',
          port_map = {
            "in_" : "d",
            "out" : "q",
          },
...
def test_reg_external_trace( do_test ):
  class VRegTrace( Component ):
    def construct( s ):
      ...
      s.out = OutPort( Bits32 )
      s.config_verilog_import = VerilatorImportConfigs(
          vl_src = get_dir(__file__)+'VRegTrace.v',
  ...
  a.elaborate()
  ipass = VerilatorImportPass()
  a.config_verilog_import.fill_missing( a )
...
def test_adder( do_test ):
  ...
  class VAdder( Component ):
    def construct( s ):
      ...
      s.config_verilog_import = VerilatorImportConfigs(vl_src = get_dir(__file__)+'VAdder.v')
  a = VAdder()
  a._test_vectors = [
    [    1,      1,     1,     3, 0 ],
    [    1,     -1,     0,     0, 1 ],
...
def test_normal_queue( do_test ):
  ...
  class VQueue( Placeholder, Component ):
    def construct( s, data_width, num_entries, count_width ):
      ...
      s.config_verilog_import = VerilatorImportConfigs(vl_src = get_dir(__file__)+'VQueue.v')
  num_entries = 1
  q = VQueue(
      data_width = 32,
