...
def test_reg( do_test ):
  ...
  class VReg( Component ):
    def construct( s ):
      ...
      s.config_verilog_import = ImportConfigs(
...
def test_adder( do_test ):
  ...
  class VAdder( Component ):
    def construct( s ):
      ...
      s.config_verilog_import = ImportConfigs(vl_src = get_dir(__file__)+'VAdder.v')
  a = VAdder()
  a._test_vectors = [
    [    1,      1,     1,     3, 0 ],
    [    1,     -1,     0,     0, 1 ],
...
def test_normal_queue( do_test ):
  ...
  class VQueue( Placeholder, Component ):
    def construct( s, data_width, num_entries, count_width ):
      ...
      s.config_verilog_import = ImportConfigs(vl_src = get_dir(__file__)+'VQueue.v')
  num_entries = 1
  q = VQueue(
      data_width = 32,
