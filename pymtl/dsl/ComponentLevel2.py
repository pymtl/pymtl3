...
class ComponentLevel2(ComponentLevel1):
    ...
    def _elaborate_read_write_func(s):
        ...
        def extract_obj_from_names(func, names):
            def expand_array_index(
                obj, name_depth, node_depth, idx_depth, idx, obj_list
            ):
                ...
                if idx_depth >= len(idx):  # exhausted, go to next level of name
                    ...
                    try:
                        ...
                        raise VarNotDeclaredError(
                            obj, idx[idx_depth], func, s, nodelist[node_depth].lineno
                        )
                    except IndexError:
                        return
                    ...
                    expand_array_index(
                        child, name_depth, node_depth, idx_depth + 1, idx, obj_list
                    )
            ...
            def lookup_variable(obj, name_depth, node_depth, obj_list):
                ...
                if name_depth >= len(obj_name):  # exhausted
                    add_all(
                        obj, obj_list, node_depth
                    )  # if this object is a list/array again...
                    return
                ...
                try:
                    ...
                    print(e)
                    raise VarNotDeclaredError(
                        obj, field, func, s, nodelist[node_depth].lineno
                    )

                s._dsl.astnode_objs[nodelist[node_depth]].append(child)

                if not idx:
                    lookup_variable(child, name_depth + 1, node_depth + 1, obj_list)
                else:
                    expand_array_index(
                        child, name_depth, node_depth + 1, 0, idx, obj_list
                    )
                            ...
                            if v in caller:  # v calls someone else there is a cycle
                                raise InvalidFuncCallError(
                                    "In class {}\nThe full call hierarchy:\n - {}{}\nThese function calls form a cycle:\n {}\n{}".format(
                                        type(
                                            m
                                        ).__name__,  # function's hostobj must be m
                                        "\n - ".join(
                                            [
                                                "{} calls {}".format(
                                                    caller[x][0].__name__, x.__name__
                                                )
                                                for x in stk
                                            ]
                                        ),
                                        "\n - {} calls {}".format(
                                            u.__name__, v.__name__
                                        ),
                                        "\n ".join(
                                            [
                                                ">>> {} calls {}".format(
            ...
            if len(wr_blks) > 1:
                raise MultiWriterError(
                    "Multiple update blocks write {}.\n - {}".format(
                        ...
                        "\n - ".join(
                            [
                                x.__name__ + " at " + repr(s._dsl.all_upblk_hostobj[x])
                                for x in wr_blks
                            ]
                        ),
                    )
                ...
                if isinstance(obj, InPort):
                    ...
                    if blk_hostobj != host:
                        raise SignalTypeError(
                            """[Type 1] Invalid read to Wire:

- Wire "{}" of {} (class {}) is read in update block
       "{}" of {} (class {}).

  Note: Please only read Wire "x.wire" in x's update block.
        (Or did you intend to declare it as an OutPort?)""".format(
                                repr(obj),
                                repr(host),
                                type(host).__name__,
                                blk.__name__,
                                repr(blk_hostobj),
                                type(blk_hostobj).__name__,
                            )
                        )
            ...
            for obj in writes:
                ...
                if isinstance(obj, InPort):
                    if host.get_parent_object() != blk_hostobj:
                        raise SignalTypeError(
                            """[Type 2] Invalid write to an input port:

- InPort "{}" of {} (class {}) is written in update block
          "{}" of {} (class {}).

  Note: Please only write to children's InPort "x.y.in", not "x.in", in x's update block.""".format(
                                repr(obj),
                                repr(host),
                                type(host).__name__,
                                blk.__name__,
                ...
                # of an instance illegal. -- IEEE
                    ...
                    if blk_hostobj != host:
                        raise SignalTypeError(
                            """[Type 3] Invalid write to output port:

- OutPort \"{}\" of {} (class {}) is written in update block
           \"{}\" of {} (class {}).

  Note: Please only write to OutPort "x.out", not "x.y.out", in x's update block.""".format(
                                repr(obj),
                                repr(host),
                                type(host).__name__,
                                blk.__name__,
...
- Wire "{}" of {} (class {}) is written in update block
  ...
  Note: Please only write to Wire "x.wire" in x's update block.
    ...
    def add_constraints(s, *args):  # add RD-U/WR-U constraints
        ...
        for (x0, x1) in args:
            if isinstance(x0, U) and isinstance(x1, U):  # U & U, same
                ...
                if isinstance(x0, RD):
                    assert (sign, x1.func) not in s._dsl.RD_U_constraints[
                        x0.var
                    ], "Duplicated constraint"
                    s._dsl.RD_U_constraints[x0.var].add((sign, x1.func))
                else:
                    assert (sign, x1.func) not in s._dsl.WR_U_constraints[
                        x0.var
                    ], "Duplicated constraint"
            ...
            for i, obj in iterable:
                ...
                if isinstance(obj, ComponentLevel1):
                    ...
                    try:
                        ...
                        raise err
                    s._dsl.swapped_signals[host_component].append(
                        (current_obj, i, obj, is_list)
                    )
    ...
    def unlock_simulation(s):
        ...
        for component, records in s._dsl.swapped_signals.items():
            for current_obj, i, obj, is_list in records:
                if is_list:
                    s._dsl.swapped_values[component] = (
                        current_obj,
                        i,
                        current_obj[i],
                        is_list,
    ...
    def get_all_update_on_edge(s):
        try:
            assert s._dsl.elaborate_top is s, (
                "Getting all update_on_edge blocks  "
                "is only allowed at top, but this API call "
                "is on {}.".format("top." + repr(s)[2:])
            )
    ...
    def get_all_upblk_metadata(s):
        try:
            assert s._dsl.elaborate_top is s, (
                "Getting all update block metadata  "
                "is only allowed at top, but this API call "
                "is on {}.".format("top." + repr(s)[2:])
            )
            return (
    ...
    def get_all_object_filter(s, filt):
        ...
        try:
            return set(
                [x for x in s._dsl.all_components | s._dsl.all_signals if filt(x)]
            )
        except AttributeError:
