...
def test_cyclic_dependency():
    class Top(ComponentLevel1):
        def construct(s):
            ...
            s.add_constraints(U(upA) < U(upB), U(upB) < U(upA))
...
def test_register_behavior():
    class Top(ComponentLevel1):
        def construct(s):
            ...
            s.add_constraints(U(up_reg) < U(up_to_sink), U(up_reg) < U(up_from_src))
...
def test_add_loopback():
    class Top(ComponentLevel1):
        def construct(s):
            ...
            s.add_constraints(
                U(upA) < U(up_to_sink_and_loop_back), U(upA) < U(up_from_src)
            )
...
def test_lots_of_fan():
    class Top(ComponentLevel1):
        def construct(s):
            ...
            s.add_constraints(U(up_reg) < U(upA), U(up_reg) < U(up_from_src))
            ...
            s.add_constraints(U(upA) < U(upB), U(upA) < U(upC))
            s.wire7 = s.wire8 = 0
            ...
            s.add_constraints(U(upB) < U(upD), U(upC) < U(upD))
            ...
            s.add_constraints(U(upD) < U(up_to_sink), U(up_to_sink) < U(up_sink))
