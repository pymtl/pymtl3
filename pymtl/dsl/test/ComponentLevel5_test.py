...
def test_simple_src_dumb_sink():
    class Top(ComponentLevel5):
        def construct(s):
            ...
            s.connect_pairs(s.src.req, s.sink.resp, s.src.req_rdy, s.sink.resp_rdy)
...
class TestSinkUp(ComponentLevel5):
  ...
  """.format(
        ...
        s.add_constraints(
            U(up_sink) < M(s.resp_), U(up_sink) < M(s.resp_rdy_)  # pipe behavior
        )
    ...
    class RecvIfcCL(Interface):
        ...
        def connect(s, other, parent):
            if isinstance(other, SendIfcCL):
                parent.connect_pairs(s.recv, other.send, s.rdy, other.rdy)
                return True
    ...
    class SendIfcCL(Interface):
        ...
        def connect(s, other, parent):
            if isinstance(other, RecvIfcCL):
                parent.connect_pairs(s.send, other.recv, s.rdy, other.rdy)
                return True
  ...
  """.format(
    ...
    class Top(ComponentLevel5):
        def construct(s):
            ...
            s.connect_pairs(s.src.req, s.sink.resp)
