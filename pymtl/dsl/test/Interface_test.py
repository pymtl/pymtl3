...
def test_nested_port_bundle():
    ...
    class Top(ComponentLevel3):
        def construct(s):
            ...
            s.src = [
                TestSourceValRdy(int, [i, i + 1, i + 2, i, i + 1, i + 2])
                for i in range(4)
            ]
        ...
        def connect(s, other, parent):
            if isinstance(other, MockSendIfc):
                parent.connect_pairs(
                    s.recv_msg, other.send_msg, s.recv_val, other.send_val
                )
                return True
    ...
    class MockSendIfc(Interface):
        ...
        def connect(s, other, parent):
            if isinstance(other, MockRecvIfc):
                parent.connect_pairs(
                    s.send_msg, other.recv_msg, s.send_val, other.recv_val
                )
                return True
    ...
    class MockRecvIfc(Interface):
        ...
        def connect(s, other, parent):
            if isinstance(other, MockSendIfc):
                ...
                parent.connect_pairs(
                    other.send_msg, m.in_, m.out, s.recv_msg, other.send_val, s.recv_val
                )
                return True
