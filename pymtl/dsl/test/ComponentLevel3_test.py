...
def test_connect_deep():
    class MuxWrap(ComponentLevel3):
        def construct(s):
            ...
            s.mux = Mux(int, 2)(out=s.out, in_={0: s.in_[0], 1: s.in_[1]}, sel=s.sel)
...
def test_deep_connect():
    class MuxWrap3(ComponentLevel3):
        def construct(s):
            ...
            s.mux1 = Mux(int, 2)(in_={0: s.in_[0], 1: s.in_[1]}, sel=s.sel)
            s.mux2 = Mux(int, 2)(in_={0: s.in_[0], 1: s.in_[1]}, sel=s.sel)
            s.mux3 = Mux(int, 2)(
                out=s.out, in_={0: s.mux1.out, 1: s.mux2.out}, sel=s.sel
            )
...
def test_connect_list_idx_call():
    class Top(ComponentLevel3):
        def construct(s):
            ...
            s.mux = Mux(int, 2)(
                out=s.sink.in_, in_={MUX_SEL_0: s.src_in0.out}, sel=s.src_sel.out
            )
