...
class DetectVarNames(ast.NodeVisitor):
    def __init__(self, upblk):
        ...
        for i, var in enumerate(upblk.__code__.co_freevars):
            try:
                self.closure[var] = upblk.__closure__[i].cell_contents
            except ValueError:
                pass
    ...
    def _get_full_name(self, node):
        ...
        while True:
            ...
            while isinstance(node, ast.Subscript) and isinstance(node.slice, ast.Index):
                ...
                if isinstance(v, ast.Num):
                    ...
                    x = v.id
                    if (
                        x in self.upblk.__globals__
                    ):  # Only support global const indexing for now
        ...
        if slices:
            assert len(slices) == 1, (
                "Multiple slices at the end of s.%s in update block %s"
                % (
                    ".".join(
                        [
                            obj_name[i][0]
                            + "".join(["[%s]" % x for x in obj_name[i][1]])
