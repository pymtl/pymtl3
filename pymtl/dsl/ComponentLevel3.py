...
class ComponentLevel3(ComponentLevel2):
    ...
    def _connect(s, other, adjacency):
        ...
        if other in adjacency[s]:
            raise InvalidConnectionError(
                "This pair of signals are already connected."
                "\n - {} \n - {}".format(s, other)
            )
    ...
    def _connect_signal_signal(s, o1, o2):
        ...
        try:
            o1_type = o1._dsl.Type
        except AttributeError:
            pass
        try:
            o2_type = o2._dsl.Type
        except AttributeError:
            pass
        ...
        try:
            ...
            assert o1_nbits == o2_nbits, "Bitwidth mismatch {} != {}".format(
                o1_nbits, o2_nbits
            )
    ...
    def _connect_interfaces(s, o1, o2):
        assert isinstance(o1, Interface) and isinstance(o2, Interface), (
            "Invalid interface connection, %s <> %s."
            % (type(s).__name__, type(other).__name__)
        )
        ...
        try:  # Catch AssertionError from _connect
            ...
            for (kw, target) in s._dsl.call_kwargs.items():
                try:
                    ...
                    raise InvalidConnectionError(
                        "{} is not a member of class {}".format(kw, s.__class__)
                    )
                ...
                if isinstance(obj, list):
                    ...
                    if not isinstance(target, dict):
                        raise InvalidConnectionError(
                            "We only support a dictionary when '{}' is an array.".format(
                                kw
                            )
                        )
                ...
                while Q:
                    ...
                    for v in adjacency[u]:
                        if v not in visited:
                            ...
                            raise InvalidConnectionError(
                                repr(v) + " is in a connection loop."
                            )
                    ...
                    try:
                        ...
                        raise MultiWriterError(
                            'Two-writer conflict "{}"{}, "{}" in the following net:\n - {}'.format(
                                ...
                                ""
                                if not obj
                                else '(as "{}" is written somewhere else)'.format(
                                    repr(obj)
                                ),
                                repr(writer),
    ...
    def _check_port_in_nets(s):
        ...
        headless = [
            signals for writer, signals in nets if writer is None
        ]  # remove None
        if headless:
            ...
            while S:
                ...
                for v in s._dsl.all_adjacency[u]:  # v is the reader
                    if v not in visited:
                        ...
                        if whost == rhost:
                            valid = (
                                isinstance(u, Signal) or isinstance(u, Const)
                            ) and (isinstance(v, OutPort) or isinstance(v, Wire))
                            if not valid:
...
- {} "{}" cannot be driven by {} "{}".
  ...
  Note: InPort x.y cannot be driven by x.z""".format(
                                        repr(rhost),
                                        type(rhost).__name__,
                                        type(v).__name__,
                                        repr(v),
...
- {} "{}" of {} (class {}) cannot be driven by {} "{}" of {} (class {}).
  ...
  Note: InPort x.y cannot be driven by x.z.a""".format(
                                        type(v).__name__,
                                        repr(v),
                                        repr(rhost),
                                        type(rhost).__name__,
...
- {} "{}" of {} (class {}) cannot be driven by {} "{}" of {} (class {}).
  ...
  Note: OutPort/Wire x.y.z cannot be driven by x.a""".format(
                                        type(v).__name__,
                                        repr(v),
                                        repr(rhost),
                                        type(rhost).__name__,
...
- {} "{}" of {} (class {}) cannot be driven by {} "{}" of {} (class {}).
  ...
  Note: Looks like the connection is fulfilled in "{}".
        OutPort/Wire x.y.z cannot be driven by x.a.b""".format(
                                        type(v).__name__,
                                        repr(v),
                                        repr(rhost),
                                        type(rhost).__name__,
                        ...
                        # 5. neither host is the other's parent nor the same.
                            ...
                            raise SignalTypeError(
                                """[Type 9] "{}" and "{}" cannot be connected:

- host objects "{}" and "{}" are too far in the hierarchy.""".format(
                                    repr(u), repr(v), repr(whost), repr(rhost)
                                )
                            )
    ...
    def _disconnect_signal_int(s, o1, o2):
        ...
        for i, net in enumerate(nets):
            ...
            if o1 in signals:  # Find the net that contains o1
                ...
                assert writer._dsl.const == o2, (
                    "Disconnecting the wrong const {} "
    ...
    def connect_pairs(s, *args):
        if len(args) & 1 != 0:
            raise InvalidConnectionError(
                "Odd number ({}) of objects provided.".format(len(args))
            )
    ...
    def add_connection(s, o1, o2):
        ...
        assert s._dsl.elaborate_top is s, (
            "Adding connection by passing objects "
            "is only allowed at top, but this API call "
            "is on {}.".format("top." + repr(s)[2:])
        )
    ...
    def disconnect(s, o1, o2):
        # TODO support string arguments and non-top s
        assert s._dsl.elaborate_top is s, (
            "Disconnecting signals by passing objects "
            "is only allowed at top, but this API call "
            "is on {}.".format("top." + repr(s)[2:])
        )
        ...
        if isinstance(o2, int):
            assert isinstance(
                o1, Signal
            ), "You can only disconnect a const from a signal."
