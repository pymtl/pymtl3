    ...
    def __init__(self, nets):
        return super(NoWriterError, self).__init__(
            "The following nets need drivers.\nNet:\n - {} ".format(
                "\nNet:\n - ".join(["\n - ".join([repr(x) for x in y]) for y in nets])
            )
        )
...
class VarNotDeclaredError(Exception):
    ...
    def __init__(self, obj, field, blk=None, blk_hostobj=None, lineno=0):
        ...
        if not self.blk:
            return super(
                VarNotDeclaredError, self
            ).__init__()  # this is just temporary message
        return super(VarNotDeclaredError, self).__init__(
            "Object {} of class {} does not have field <{}> ({}.{})\n - Occurred at Line {} of {} at {} (class {})".format(
                ...
                blk.__name__,
                repr(blk_hostobj),
                blk_hostobj.__class__.__name__,
            )
        )
...
class UpblkFuncSameNameError(Exception):
    ...
    def __init__(self, name):
        return super(UpblkFuncSameNameError, self).__init__(
            " Cannot declare two update blocks/functions with the same name {}".format(
                name
            )
        )
