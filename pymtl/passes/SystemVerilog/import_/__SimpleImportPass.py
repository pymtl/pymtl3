...
class SimpleImportPass( BasePass ):
  ...
  def __call__( s, model ):
    ...
    try:
      assert model._pass_systemverilog_translation.translated,\
        "Component should be translated first!"
    ...
    sense_group = s.load_ssg_file( ssg_name )
  ...
  def create_verilator_py_wrapper(
    ...
    # The (optional) sequential block
    seq_upblk = s.generate_seq_upblk( model, array_dict, sense_group )
...
"""
    ...
    comb_upblks = s.generate_comb_upblks( model, array_dict, sense_group )
  ...
  def generate_seq_upblk( s, model, array_dict, sense_group ):
    """Return the seq upblk in string according to sense_group"""

    if sense_group is None: return ''

    # Check if the model is purely combinational
    seq_in_ports = []
    ...
    for out, in_ in sense_group:
      for in_port in in_:
        if in_port.startswith( ('S', 'B') ):
          seq_in_ports.append( in_port[1:] )

    # Purely combinational models do not need sequential blocks
    if seq_in_ports == []: return ''

    seq_upblk_tplt = """
    @s.update_on_edge
    def tick_imported_model():
      {register_inputs}
      s._ffi_m.clk[0] = 0
      s._ffi_inst.eval( s._ffi_m )
      s._ffi_m.clk[0] = 1
    ...
    for port in seq_in_ports:
      set_inputs.extend(s.set_input_stmt( model.__dict__[ port ], array_dict ))

  def generate_comb_upblks( s, model, array_dict, sense_group ):
    """Return a list of comb upblks in string according to sense_group"""
    comb_upblk = """
    @s.update
    def {upblk_name}():
      # set inputs
    ...
    if sense_group is None:
      ...
      for port in model.get_output_value_ports():
        ...
        comb = s.set_output_stmt( port, array_dict )
        set_comb.extend( comb )

      ret.append( comb_upblk.format(
  ...
  def load_ssg_file( s, ssg_name ):
    """Load the .ssg file and return the sensitivity group"""
  ...
  def generate_port_assignment( s, port, array_dict ):
    ret = []
    name = port._dsl.my_name

    if '[' in name:
      name = get_array_name( name )
      ret.append( 'for _x in xrange({array_range}):'.\
          format( array_range = array_dict[ name ] ) 
      )
      for idx, offset in s.get_indices( port ):
        ret.append(
          '  {lhs_prefix}{py_name}[_x]{offset} = {rhs_prefix}{v_name}[_x][{idx}]'.\
          format(
            lhs_prefix = lhs_prefix, rhs_prefix = rhs_prefix,
            v_name = port.verilator_name, py_name = name,
  ...
  def set_output_stmt( s, port, array_dict ):
    """Generate the list of vars that should be called in update blocks"""
    # This function is based on its counterpart from PyMTL v2
    ...
    if '[' in name:
      name = get_array_name( name )
      comb.append( 'for _x in xrange({array_range}):'.\
          format( array_range = array_dict[ name ] ) 
      )
      for idx, offset in s.get_indices( port ):
        comb.append(
