...
class SystemVerilogTranslationPass(BasePass):
    def __call__(self, top):
        ...
        with open(temp_file, "w+") as fd:
            for obj in all_components:
                ret += self.translate_component(
                    obj,
                    connections_self_self[obj],
    ...
    def translate_component(
        self, m, connections_self_self, connections_self_child, connections_child_child
    ):
        ...
        input_strs = [
            "input logic{}".format(gen_signal_width_name(x))
            for x in sorted(m.get_input_value_ports(), key=repr)
        ]

        output_strs = [
            "output logic{}".format(gen_signal_width_name(x))
        ...
        wire_strs = [
            "logic{};".format(gen_signal_width_name(x))
...
class FuncUpblkTranslator(ast.NodeVisitor):
    ...
    def enter(self, blk, ast):
        ...
        for i, var in enumerate(blk.__code__.co_freevars):
            try:
                self.closure[var] = blk.__closure__[i].cell_contents
            except ValueError:
                pass
    ...
    def visit_Assign(self, node):  # a = ...
        ...
        return "".join(
            [
                self.newline("{} {} {};".format(self.visit(lhs), assign, rhs))
    ...
    def visit_BoolOp(self, node):
        return "{}".format(
            " {} ".format(opmap[type(node.op)]).join(
                [self.visit_expr_wrap(expr) for expr in node.values]
            )
        )

    def visit_BinOp(self, node):
        return "{} {} {}".format(
            self.visit_expr_wrap(node.left),
            opmap[type(node.op)],
            self.visit_expr_wrap(node.right),
        )

    def visit_UnaryOp(self, node):
        return "{}{}".format(opmap[type(node.op)], self.visit_expr_wrap(node.operand))

    def visit_IfExp(self, node):
        return "{} ? {} : {}".format(
            self.visit_expr_wrap(node.test),
    ...
    def visit_Call(self, node):
        ...
        try:
            ...
            # TODO max, min,
            return "{}( {} )".format(
                call.__name__, ", ".join([self.visit(arg) for arg in node.args])
            )
