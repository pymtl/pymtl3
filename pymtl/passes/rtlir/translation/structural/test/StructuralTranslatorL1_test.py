...
from pymtl.passes.rtlir.translation.structural.StructuralTranslatorL1 import (
...
def local_do_test( m ):
  ...
  try:
    ...
    # assert reduce(lambda r, o: r or decl_ports == o, m._ref_ports, False)
    decl_wires = tr.structural.decl_wires[m]
    assert decl_wires == m._ref_wires
    # assert reduce(lambda r, o: r or decl_wires == o, m._ref_wires, False)
    decl_consts = tr.structural.decl_consts[m]
    assert decl_consts == m._ref_consts
    # assert reduce(lambda r, o: r or decl_consts == o, m._ref_consts, False)
    connections = tr.structural.connections[m]
    assert connections == m._ref_conns
    # assert reduce(lambda r, o: r or connections == o, m._ref_conns, False)
