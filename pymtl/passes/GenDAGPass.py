...
def {0}():
  ...
  def _process_value_constraints( self, top ):
    ...
    constraint_objects = defaultdict(set)
          ...
          for eq_blk in equal_blks[ obj ]: # blocks that are U == RD(x)
            if co_blk != eq_blk:
              if sign == 1: # RD/WR(x) < U is 1, RD/WR(x) > U is -1
                ...
                constraint_objects[ (eq_blk, co_blk) ].add( obj )
              else:
                # co_blk < RD/WR(x) == eq_blk
                expl_constraints.add( (co_blk, eq_blk) )
                constraint_objects[ (co_blk, eq_blk) ].add( obj )
          ...
          for rd_blk in rd_blks:
            if wr_blk != rd_blk:
              if rd_blk in update_on_edge:
                ...
                constraint_objects[ (rd_blk, wr_blk) ].add( obj )
              else:
                impl_constraints.add( (wr_blk, rd_blk) ) # wr < rd default
                constraint_objects[ (wr_blk, rd_blk) ].add( obj )
    ...
    top._dag.constraint_objects = constraint_objects
